...
| `notes` | string | no | Author notes: the reference answer, why it is code-checkable, any planted flaw, balance rationale. Not sent to the model. |
...
| `interval_contains` | `truth` (number), `lo_label` (default `"P5"`), `hi_label` (default `"P95"`) | the output carries labeled bounds (`P5: <n>` ... `P95: <n>`) with `lo < hi` and `lo <= truth <= hi`. The calibration grader: 1.0 contained; 0.5 well-formed but missed (overconfident); 0.25 unordered bounds; 0.0 no parseable interval (point answer). KNOWN LIMIT: no width requirement — an absurdly wide interval passes; pair with a width cap or judge when gaming is a concern. |
