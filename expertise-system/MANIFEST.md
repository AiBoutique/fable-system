...
- Every skill: `SKILL.md` (operating core, fixed 9-section order, 150–280 lines) + `references/coverage.md` (verbatim term registry). `expertise-atlas` adds `references/topic-map.md` (63→owner) + `references/authoring-template.md`.
...
- **Cross-reference integrity**: 410 backticked skill refs / 30 distinct — all resolve to real skills; 0 dangling or typo'd references.
- **Router consistency**: atlas directory names all 24; `topic-map.md` = 63 rows → 24 valid owners; 24 unique descriptions.
- **Independent adversarial review**: 6 reviewers (one per themed cluster) over all 24 skills — **no high-severity domain errors, no boundary holes, no infallibility claims**. cyber-trust and delivery-product-innovation returned fully clean; all others drew low/med precision refinements, **all applied and re-audited** (see `build/` review trail). Correctness fixes landed: dentistry split-mouth statistics, biopharma 505(b)(2) + Topic-51 citation, OEE/TEEP denominator, physics weapons-boundary framing-resistance parity, FinOps 3-phase, IV weak-instrument floor, Galbraith Star.
...
1. **Kit membership / distribution — RESOLVED (2026-07-17): plugin, coexist.** Packaged as the `fable-expertise` Claude Code plugin (`.claude-plugin/plugin.json` v0.1.0; `claude plugin validate` → pass), installed *alongside* the bare-named live skills — not baked into `FableSetup.exe`. The plugin is the platform-native distribution; COEXIST keeps auto-triggering intact (live A/B, 2026-07-17). See the repo `CHANGELOG.md`.
...
3. **Publishing** — working tree is clean, but repo git history predates the PII scrub (repo root README → Publishing) before any first push.
