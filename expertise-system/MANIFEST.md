...
1. **Kit membership / distribution — RESOLVED (2026-07-17): plugin, coexist.** Packaged as the `fable-expertise` Claude Code plugin (`.claude-plugin/plugin.json` v0.1.0; `claude plugin validate` → passes with one intentional warning, the omitted `author` field), installed *alongside* the bare-named live skills — not baked into `FableSetup.exe`. The plugin is the platform-native distribution; COEXIST keeps auto-triggering intact (live A/B, 2026-07-17). See the repo `CHANGELOG.md`.
