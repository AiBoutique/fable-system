...
- Regulatory and framework texts before any compliance claim: EU AI Act (current consolidated text, obligations, application dates), NIST AI RMF (current version and profiles), OWASP LLM Top 10 and agentic-security guidance — all evolve; retrieve, date, cite.
...
- Risk-tier on the EU AI Act structure as scaffold — prohibited / high-risk / limited / minimal plus general-purpose-AI obligations; retrieve the current consolidated text and application dates before any readiness claim; document high-risk-system assessments. Run the management system on NIST AI RMF functions (Govern/Map/Measure/Manage — retrieve current version and profiles).
