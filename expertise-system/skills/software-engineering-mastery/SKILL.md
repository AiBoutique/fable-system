...
description: "Master-grade software, AI engineering, and DevSecOps — algorithms and data structures, the major languages (Python, TypeScript, Java, Go, Rust, SQL), software architecture (microservices, DDD), API design, databases, data engineering, frontend/backend/mobile, cloud-native and Kubernetes, infrastructure as code, CI/CD, testing at every level (TDD, fuzz), debugging and observability, performance, reliability and SRE, secure coding and supply-chain security (SBOM, SLSA), AI engineering (RAG, agents, MCP, evals, LLMOps), legacy modernization, code review. Use for designing, writing, reviewing, debugging, securing, testing, and shipping software and AI systems."
---
...
- SSDF (NIST SP 800-218) and CISA Secure by Design as the org-level checklists; when the artifact is a generative-AI or dual-use foundation model, add the SP 800-218A community profile, which layers model-development practices (training-data provenance, model weights as release artifacts, evaluation before release) onto the same SSDF spine rather than replacing it — the base framework and its profiles version independently, so retrieve both. Memory-safe languages preferred for new systems code, with the unsafe-interop boundary named.
