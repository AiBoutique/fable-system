...
The standing rule this skill enforces: a project container holds exactly three top-level folders (plus, where present, the container's own CLAUDE.md rule file and a harness-created `.claude\` project-config dir — both expected, never filed away as strays) —
...
4. **Move with git-awareness**: files tracked by a repo move with `git mv` inside it — never plain-move tracked files out of a repo; the resulting change is left staged-or-noted for the user's commit (committing itself follows the session's commit rules — never uninvited); a tracked file classified private cannot leave by `git mv` (it cannot cross the worktree boundary): copy it to `private\`, then `git rm` it in the repo, leave that staged, and treat the case as mandatorily triggering the history sweep in step 6; a `.git` directory always moves together with its whole tree, never split from it; nothing classified private ever crosses into a repo.
