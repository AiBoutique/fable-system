...
- **No personal data**: the public kit ships **no project memory** and an **empty MCP template**. Populate `mcp-servers.template.json` from your own machine with `node tools\export-mcp-template.js "$env:USERPROFILE\.claude.json" mcp-servers.template.json` (PowerShell syntax; in cmd.exe use `%USERPROFILE%` instead — under PowerShell the `%…%` form is passed to node as a literal string) — secrets are auto-redacted to `<<SET-ME>>` and home paths tokenized to `<<HOME>>` (never commit a populated template without checking it).
...
| `tools\run-selftest.ps1` | The full installer harness (190 assertions; the TOTAL line it prints is the count of record) — sandboxes under %TEMP%, needs PowerShell 7 + Node.js + Git Bash + `claude` on PATH. |
...
| `claude-home\settings.json` | Three hooks + three benign prefs: SessionStart standing order; UserPromptSubmit prompt-scoped classifier v10; SubagentStart verify-order injection. |
