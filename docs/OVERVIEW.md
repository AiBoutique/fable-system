...
| **Self-verifying installer** | One-click Windows exe with a SHA-256 integrity gate, backup-then-merge (never clobbers foreign config), previous-version cleanup via an install ledger (stale old-kit files removed and backed up; your edits kept), and a 190-assertion install self-test. |
